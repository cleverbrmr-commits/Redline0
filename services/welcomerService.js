...
function normalizeWelcomerConfig(rawConfig = {}) {
  return {
    ...
    style: rawConfig.style || 'blue-premium',
    messageLines: normalizeMessageLines(rawConfig.messageLines),
    highlightChannelId: rawConfig.highlightChannelId || null,
    backgroundImageUrl: rawConfig.backgroundImageUrl || null,
    textColor: rawConfig.textColor || null,
    showMemberCount: rawConfig.showMemberCount !== false,
    showAvatar: rawConfig.showAvatar !== false,
    showJoinText: rawConfig.showJoinText !== false,
...
function buildWelcomerStatusEmbed(config) {
  ...
  return makeInfoEmbed({
    ...
    fields: [
      ...
      { name: 'Show Member Count', value: welcomer.showMemberCount ? 'Enabled' : 'Disabled', inline: true },
      { name: 'Show Avatar', value: welcomer.showAvatar ? 'Enabled' : 'Disabled', inline: true },
      { name: 'Show Join Text', value: welcomer.showJoinText ? 'Enabled' : 'Disabled', inline: true },
      { name: 'Auto Role', value: welcomer.autoRoleId ? `<@&${welcomer.autoRoleId}>` : 'Not configured', inline: true },
      { name: 'Message Preview', value: welcomer.messageLines.map((line) => `• ${trimText(line, 120)}`).join('\n'), inline: false },
    ],
